...
Now it's time to go a little bit deeper in the application and move to the `AddTaskHandler` use case. What we expect from this UseCase is that is uses the information it receives to create a task and stores it in TaskRepository.

To create a task, we'll need to give it an ID, which we're going to ask from the repository itself
